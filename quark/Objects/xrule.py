...
class XRule:
    ...
    def get_json_report(self, rule_obj):
        """
        Output report by json

        :return: crime, confidence, score, weight
        """
        confidence = rule_obj.check_item.count(True) * 20
        conf = rule_obj.check_item.count(True)
        weight = rule_obj.get_score(conf)
        score = rule_obj.yscore
        return rule_obj.crime, confidence, score, weight
